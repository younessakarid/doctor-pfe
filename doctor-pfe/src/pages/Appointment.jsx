...
function Appointment() {
  ...
  if (loading) {
    return <div>Chargement des informations du docteur...</div>
  }
  if (!docInfo) {
    return <div>Docteur introuvable.</div>
  }
  ...
  return (
    ...
    <div className='flex flex-col sm:flex-row gap-4 p-4 sm:p-8'>
      <div className='w-full sm:w-1/3 md:w-1/4'>
        <img src={docInfo.image} className='bg-[#1e84b5] w-full rounded-lg' alt={docInfo.name} />
      </div>
  ...
  <div className="flex justify-center sm:justify-start mt-6 mb-6">
    <button className='bg-[#1e84b5] text-white text-sm font-light px-8 sm:px-14 py-2.5 sm:py-3 rounded-full hover:bg-[#1976a2] transition-colors w-full sm:w-auto'>
      Prendre rendez-vous
    </button>
  </div>
</div>
